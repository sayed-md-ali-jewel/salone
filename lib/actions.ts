...
function isStaffPaymentForPeriod(
  expense: { title: string; notes: string | null },
  employeeId: string,
  employeeName: string,
  periodLabel: string,
) {
  ...
  const matchesEmployee =
    expense.title.includes(employeeName) ||
    notes.includes(employeeName) ||
    notes.includes(`Employee ID: ${employeeId}`);
...
export async function createCustomer(formData: FormData) {
  return runAction("createCustomer", formData, async () => {
    ...
    const previousDueNote = String(
      formData.get("previousDueNote") || "",
    ).trim();
...
export async function deleteCustomer(formData: FormData) {
  return runAction("deleteCustomer", formData, async () => {
    ...
    if (!hasDatabaseUrl() || !isObjectId(id))
      redirect("/customers?status=customer-invalid");
...
export async function createCustomerPayment(formData: FormData) {
  return runAction("createCustomerPayment", formData, async () => {
    ...
    const paymentDate = paymentDateValue
      ? validDate(`${paymentDateValue}T00:00:00`)
      : new Date();
    const notes = String(formData.get("notes") || "").trim();

    if (
      !hasDatabaseUrl() ||
      !isObjectId(customerId) ||
      amount === null ||
      amount <= 0 ||
      !paymentDate ||
      !notes
    ...
    const paidAmount = customer.customerPayments.reduce(
...
export async function createEmployee(formData: FormData) {
  return runAction("createEmployee", formData, async () => {
    ...
    const joiningDate = joiningDateValue
      ? validDate(`${joiningDateValue}T00:00:00`)
      : null;
    const salaryType =
      String(formData.get("salaryType")) === "PERCENTAGE"
        ? "PERCENTAGE"
        : "MONTHLY";
      ...
      updates: [
        {
          ...
          u: {
            $set: {
              ...
              monthlySalary: salaryType === "MONTHLY" ? monthlySalary : null,
              commissionRate:
                salaryType === "PERCENTAGE" ? commissionRate : null,
              dueSalary,
              dueSalaryNote,
              updatedAt: mongoDate(),
            },
...
export async function createExpense(formData: FormData) {
  return runAction("createExpense", formData, async () => {
    ...
    const expenseDate = expenseDateValue
      ? validDate(`${expenseDateValue}T00:00:00`)
      : new Date();
    const notes = String(formData.get("notes") || "").trim();
...
export async function payEmployee(formData: FormData) {
  return runAction("payEmployee", formData, async () => {
    ...
    const paymentDate = paidDate
      ? validDate(`${paidDate}T00:00:00`)
      : new Date();

    if (
      !hasDatabaseUrl() ||
      !isObjectId(employeeId) ||
      customAmount === null ||
      customAmount <= 0 ||
      !paymentDate
    ) {
    ...
    const serviceEntries = range
      ? employee.serviceEntries.filter(
          (entry) =>
            entry.serviceDate >= range.from && entry.serviceDate < range.to,
        )
      : employee.serviceEntries;
    const commissionPayment =
      employee.salaryType === "PERCENTAGE"
        ? serviceEntries.reduce(
            (total, entry) => total + Number(entry.commissionAmount),
            0,
          )
        : 0;
    ...
    const previousPayments = await prisma.expense.findMany({
      where: { category: "SALARY" },
    });
    const alreadyPaid = previousPayments
      .filter((expense) =>
        isStaffPaymentForPeriod(
          expense,
          employeeId,
          employee.name,
          periodLabel,
    ...
    await insertOne("Expense", {
      ...
      notes: [
        ...
        remainingAfterPayment > 0
          ? `After payment amount: ${remainingAfterPayment.toFixed(2)}.`
          : "",
      ]
        .filter(Boolean)
        .join(" "),
...
export async function createServiceEntry(formData: FormData) {
  return runAction("createServiceEntry", formData, async () => {
    ...
    const serviceIds = formData
      ...
      .filter(Boolean);
    const fallbackServiceId = String(formData.get("serviceId") || "");
    const selectedServiceIds = serviceIds.length
      ? serviceIds
      : [fallbackServiceId].filter(Boolean);
    const employeeId = String(formData.get("employeeId") || "");
    ...
    if (
      ...
      selectedServiceIds.some((serviceId) => !isObjectId(serviceId))
    ) {
      redirect("/entries?status=entry-invalid");
    }

    const [selectedServices, employee] = await Promise.all([
      prisma.service.findMany({ where: { id: { in: selectedServiceIds } } }),
      prisma.employee.findUnique({ where: { id: employeeId } }),
    ]);

    if (selectedServices.length !== selectedServiceIds.length || !employee)
      redirect("/entries?status=entry-invalid");

    const extraTaskAmount = validMoney(formData.get("extraTaskAmount"));
    const discountType = String(formData.get("discountType") || "AMOUNT");
    const discountValue = validMoney(formData.get("discountValue"));
    const serviceDateValue = String(formData.get("serviceDate") || "");
    const serviceDate = serviceDateValue
      ? validDate(`${serviceDateValue}T00:00:00`)
      : new Date();

    if (
      extraTaskAmount === null ||
      extraTaskAmount < 0 ||
      discountValue === null ||
      discountValue < 0 ||
      !["AMOUNT", "PERCENTAGE"].includes(discountType) ||
    ...
    const notes = String(formData.get("notes") || "").trim();
    const grossAmounts = selectedServiceIds.map((serviceId, index) => {
      const service = servicesById.get(serviceId);
      return Number(service?.price || 0) + (index === 0 ? extraTaskAmount : 0);
    });
    const subtotalAmount = grossAmounts.reduce((total, amount) => total + amount, 0);
    const discountAmount = discountType === "PERCENTAGE"
      ? (subtotalAmount * discountValue) / 100
      : discountValue;

    if (!Number.isFinite(subtotalAmount) || subtotalAmount <= 0 || discountAmount > subtotalAmount) {
      redirect("/entries?status=entry-invalid");
    }
    ...
    await Promise.all(
      selectedServiceIds.map((serviceId, index) => {
        ...
        const serviceDiscountAmount = subtotalAmount > 0
          ? (discountAmount * grossAmount) / subtotalAmount
          : 0;
        const amount = grossAmount - serviceDiscountAmount;
        const commissionAmount = (amount * rate) / 100;

        return insertOne("ServiceEntry", {
          customerId: oid(customerId),
          ...
          notes: [
            notes,
            index === 0 && extraTaskAmount > 0
              ? `Extra Service Fee: ${extraTaskAmount.toFixed(2)}.`
              : "",
            index === 0 && discountAmount > 0
...
export async function updateServiceEntry(formData: FormData) {
  return runAction("updateServiceEntry", formData, async () => {
    ...
    if (!Number.isFinite(amount) || amount <= 0 || !serviceDate)
      redirect("/entries?status=entry-invalid");
...
export async function createUser(formData: FormData) {
  return runAction("createUser", formData, async () => {
    ...
    const role =
      roleValue === "ADMIN" || roleValue === "MANAGER" ? roleValue : "CASHIER";

    if (!name || !email || password.length < 6)
      settingsRedirect("user-invalid", "users");
...
export async function changeOwnPassword(formData: FormData) {
  return runAction("changeOwnPassword", formData, async () => {
    ...
    if (newPassword.length < 6 || newPassword !== confirmPassword)
      settingsRedirect("password-invalid");
